...
impl VaultContract {
    ...
    pub fn initialize(env: Env, admin: Address) -> Result<(), VaultError> {
        if env.storage().instance().has(&DataKey::Admin) {
            return Err(VaultError::AdminNotSet); // Using existing error instead of AlreadyInitialized
        }
    ...
    pub fn deposit(
        ...
        Self::internal_transfer_from(&env, &asset, &from, &env.current_contract_address(), amount)
    }
    ...
    // ====================================================================
    // Withdraw Available (Unlocked Funds)
    // ====================================================================
    ...
    pub fn withdraw_available(
        ...
        if amount > available {
            return Err(VaultError::InsufficientBalance); // Using existing error
        }
    ...
    pub fn set_protocol_asset(env: Env, admin: Address, asset: Address) -> Result<(), VaultError> {
        ...
        if Self::is_whitelisted(&env, &asset) {
            return Err(VaultError::AssetAlreadyWhitelisted);
        }
    ...
    pub fn whitelist_asset(env: Env, admin: Address, asset: Address, allowed: bool) -> Result<(), VaultError> {
        ...
        if allowed && Self::is_whitelisted(&env, &asset) {
            return Err(VaultError::AssetAlreadyWhitelisted);
        }
    ...
    pub fn set_pause(env: Env, admin: Address, paused: bool) -> Result<bool, VaultError> {
        admin.require_auth();
        let pause_admin = Self::get_admin(&env)?;

        if pause_admin != admin {
            return Err(VaultError::NotAuthorized);
        }

        if paused && Self::is_paused(&env) {
            return Err(VaultError::ContractPaused);
        }
    ...
    fn ensure_trustline_exists(env: &Env, recipient: &Address, asset: &Address) -> Result<(), VaultError> {
        ...
        match token_client.try_balance(recipient) {
            Ok(_) => Ok(()),
            Err(_) => Err(VaultError::TransferFailed), // Using existing error
        }
    }
    ...
    fn internal_transfer_from(
        ...
        client.transfer_from(from, from, to, &amount);
        Ok(())
    }
}
